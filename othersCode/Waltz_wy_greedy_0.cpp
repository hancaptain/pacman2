...
#define MAX_GENERATOR_COUNT 4  // 每个象限1个， 一共有4个豆子发生器
...
const int death_score = 3;

const int timeLimit = 0.9 * CLOCKS_PER_SEC;
...
namespace Pacman
{
    ...
    class GameField
    {
       ...
       public:
            ...
            if (Threat)
            {
                for (_ = 0; _ < MAX_PLAYER_COUNT; _++)
                {
                    ...
                    for (player = 0; player < MAX_PLAYER_COUNT; player++)
                        if (fieldContent[_p.row][_p.col] &
                            playerID2Mask[player])
        ...
        } shortPath[FIELD_MAX_HEIGHT][FIELD_MAX_WIDTH];
        ...
        } deadGrid[FIELD_MAX_HEIGHT][FIELD_MAX_WIDTH];
        ...
        bool notInDeadEnd(int myID, Direction dir)
        {
            ...
            for (int i = 0; i < MAX_PLAYER_COUNT; i++)
            {
                if (players[i].dead || myID == i ||
                    ...
                    continue;
                if (shortPath[ey][ex].dist[players[i].row][players[i].col] -
                            1 <=
            ...
            if (visited[_y][_x])
            {
                ...
                for (int i = 0; i < generatorCount; i++)
                {
                    ...
                    for (int d = 0; d < 8; d++)
                    {
                        ...
                        temp +=
                            1.0 / std::max(shortPath[_y][_x].dist[ny][nx], 1);
                    }
                    ToGenerator += temp / 8;
                }
...
Pacman::Player o;
double Evaluation(Pacman::GameField &gF, int myID)
{
    //考虑：豆子距离，当前玩家吃到的豆子数，到生成器的时空关系
    double FruitExpectation = 0;
    int Node_small = 0, Node_large = 0, rank = 0;
    Pacman::Player p = gF.players[myID];
    ...
    for (int i = 0; i < gF.height; i++)
    {
        ...
        {
            if (gF.fieldContent[i][j] & Pacman::smallFruit)
            {
                ++Node_small;
                FruitExpectation +=
                    1.0 / std::max(gF.shortPath[p.row][p.col].dist[i][j], 1);
            }
            else if (gF.fieldContent[i][j] & Pacman::largeFruit)
            {
                ++Node_large;
                FruitExpectation +=
                    (rank + 1) /
    ...
    for (int i = 0; i < gF.generatorCount; i++)
    {
        ...
        for (int d = 0; d < 8; d++)
        {
            int ny = (gF.generators[i].row + Pacman::dy[d] + gF.height) %
                     gF.height,
                nx = (gF.generators[i].col + Pacman::dx[d] + gF.width) %
                     gF.width;
...
double GreedySearch(Pacman::GameField &gF, int depth, int myID, int next)
{
    ...
    {
        double bestScore = 0;
        for (Pacman::Direction _d = Pacman::Direction::up; _d < 4; ++_d)
        {
            if (gF.ActionValid(myID, _d) && gF.ActionWise(myID, _d))
            {
                for (int i = 0; i < MAX_PLAYER_COUNT; i++)
                    gF.actions[i] = Pacman::Direction::stay;
                gF.actions[myID] = _d;
...
int main()
{
    ...
    o = gameField.players[myID];
    gameField.FindAllPath();
    gameField.CheckGrid();

    Pacman::Direction ans = Pacman::Direction::stay, finalAns = ans;

    for (int i = 0; i < MAX_PLAYER_COUNT; i++)
    {
        if (gameField.players[i].dead || i == myID) continue;
        if (gameField.players[i].strength < gameField.players[myID].strength &&
            gameField
                    .deadGrid[gameField.players[i].row]
                    ...
                    .type == 1)
        {
            ...
            int _x = gameField
                         .deadGrid[gameField.players[i].row]
                                  [gameField.players[i].col]
                         .entry.x;
            if (gameField.shortPath[_y][_x].dist[gameField.players[i].row]
                                                [gameField.players[i].col] >=
                    gameField.shortPath[_y][_x]
                        .dist[gameField.players[myID].row]
                             [gameField.players[myID].col] &&
                gameField.ActionWise(myID,
                                     gameField
                                         .shortPath[gameField.players[myID].row]
                                                   [gameField.players[myID].col]
                                         .dir[gameField.players[i].row]
                                             [gameField.players[i].col]))
            {
                ans = gameField
                          .shortPath[gameField.players[myID].row]
    ...
    if (ans != Pacman::Direction::stay)
    {
        gameField.WriteOutput(ans, "Got'ya!!!", data, globalData);
        return 0;
    }
    ...
    while (!overtime)
    {
        ...
        double maxB = -100000;
        depth++;
        for (Pacman::Direction _d = Pacman::Direction::stay; _d < 4; ++_d)
        {
            if (gameField.ActionValid(myID, _d) &&
                gameField.ActionWise(myID, _d) &&
                gameField.notInDeadEnd(myID, _d))
            {
                Can[int(_d) + 1] = 1;
                for (int i = 0; i < MAX_PLAYER_COUNT; i++)
                    gameField.actions[i] = Pacman::Direction::stay;
                gameField.actions[myID] = _d;
                bool hasNext = gameField.NextTurn();
                Best[int(_d) + 1] =
                    GreedySearch(gameField, depth, myID, hasNext);
                gameField.PopState();
    ...
    strings[2] = "我靠( ‵o′)凸";
    strings[3] = "（*@ο@*） 哇～";
    gameField.WriteOutput(finalAns, strings[rand() % 4].c_str(), data,
                          globalData);
