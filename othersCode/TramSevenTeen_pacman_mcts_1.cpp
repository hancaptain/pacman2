...
namespace Pacman
{
    ...
    class GameField
    {
       ...
       public:
        ...
        int feature0, feature1[4], feature2[6], feature3[4], feature4[4],
            feature5[4][32];
        int belong[FIELD_MAX_HEIGHT][FIELD_MAX_WIDTH];
        int distToFruit[32][FIELD_MAX_HEIGHT][FIELD_MAX_WIDTH];
        bool setDistFruit;

        void bfsFruit()
        {
            if (setDistFruit) return;
                ...
                for (int d = 0; d < 8; ++d)
                {
                    ...
                    for (int h = 0; h < height; h++)
                        for (int w = 0; w < width; w++)
                            distToFruit[index][h][w] = 500;
                    distToFruit[index][r][c] = 0;
                    while (!bfs.empty())
                    {
                        FieldProp curGrid = bfs.front();
                        for (int dir = 0; dir < 4; dir++)
                        {
                            // 有墙这个方向走不了
                            if (fieldStatic[curGrid.row][curGrid.col] &
                                direction2OpposingWall[dir])
                                continue;
                            FieldProp nextGrid;
                            nextGrid.row =
                                (curGrid.row + dy[dir] + height) % height;
                            nextGrid.col =
                                (curGrid.col + dx[dir] + width) % width;
                            // 没遍历过
                            if (distToFruit[index][nextGrid.row]
                                           [nextGrid.col] == 500)
                            {
                                distToFruit[index][nextGrid.row][nextGrid.col] =
                                    distToFruit[index][curGrid.row]
                                               [curGrid.col] +
                                    1;
                                bfs.push(nextGrid);
                            }
        ...
        void computeFeature()
        {
            // f0: Area, not include fruit generator
            feature0 = height * width - 4;

            // f1: Strenth
            for (int i = 0; i < 4; i++) feature1[i] = players[i].strength;

            // f2: Pairwise Distance

            // f3: Number of fruit in control (nearest to player[i])

            // f4: Number of area in conrtrol (nearest to player[i])
            int dPlayer[4][4];
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++) dPlayer[i][j] = 500;
            int shortDist[FIELD_MAX_HEIGHT][FIELD_MAX_WIDTH];
            ...
            for (int i = 0; i < 4; i++)
            {
                head[i].row = players[i].row;
                head[i].col = players[i].col;
                bfs.push(head[i]);
                shortDist[head[i].row][head[i].col] = 0;
                belong[head[i].row][head[i].col] = i;
            }
            int numExpand = bfs.size();
            int dist = 0;
            while (numExpand > 0)
            {
                dist++;
                for (int i = 0; i < numExpand; i++)
                {
                    FieldProp curGrid = bfs.front();
                    //如果是私有领地
                    if (belong[curGrid.row][curGrid.col] > -1)
                    {
                        for (int dir = 0; dir < 4; dir++)
                        {
                            // 有墙这个方向走不了
                            ...
                            nextGrid.col =
                                (curGrid.col + dx[dir] + width) % width;
                            if (belong[nextGrid.row][nextGrid.col] ==
                                belong[curGrid.row][curGrid.col])  // 自家领地
                                continue;
                            else if (belong[nextGrid.row][nextGrid.col] >
                                     -1)  // 别人已经占领的领地
                            {
                                // 如果小于最小距离则更新
                                int p1 = belong[curGrid.row][curGrid.col];
                                int p2 = belong[nextGrid.row][nextGrid.col];
                                // 距离相等，公共领地
                                if (shortDist[nextGrid.row][nextGrid.col] ==
                                    dist)
                                    belong[nextGrid.row][nextGrid.col] = -1;
                                if (dist +
                                        shortDist[nextGrid.row][nextGrid.col] <
                                    dPlayer[p1][p2])
                                {
                                    dPlayer[p1][p2] = dPlayer[p2][p1] =
                                        dist +
                                        shortDist[nextGrid.row][nextGrid.col];
                                }
                            }
                            else if (belong[nextGrid.row][nextGrid.col] == -1 &&
                                     shortDist[nextGrid.row][nextGrid.col] ==
                                         500)  //未开发
                            {
                                bfs.push(nextGrid);
                                shortDist[nextGrid.row][nextGrid.col] = dist;
                                belong[nextGrid.row][nextGrid.col] =
                                    belong[curGrid.row][curGrid.col];
                            }
                        }
                    }
                    bfs.pop();
            ...
            feature2[0] = dPlayer[0][1];
            feature2[1] = dPlayer[0][2];
            feature2[2] = dPlayer[0][3];
            feature2[3] = dPlayer[1][2];
            feature2[4] = dPlayer[1][3];
            feature2[5] = dPlayer[2][3];
            for (int r = 0; r < height; r++)
            {
                for (int c = 0; c < width; c++)
                {
                    int i = belong[r][c];
                    if (i == -1) continue;
                    if (fieldContent[r][c] & smallFruit)
                        feature3[i]++;
                    else
                        feature4[i]++;
                }
            ...
            // f5: Distance between player[i] and generatingFruit[j]
            if (setDistFruit)
            {
                for (int i = 0; i < 4; i++)
                {
                    for (int f = 0; f < 32; f++)
                    {
                        int r = players[i].row;
                        int c = players[i].col;
                        feature5[i][f] = distToFruit[f][r][c];
                    }
                }
            }
        }
        ...
        // 完成决策，输出结果。
        // action 表示本回合的移动方向，stay
        // 为不移动，shoot开头的动作表示向指定方向施放技能
        // tauntText
...
time_t dbegin,
    dstop;  //记录程序开始算下一个节点时的开始时间和结束时间，用来卡搜索深度
int count;
...
struct state
{
    ...
    state()
    {
        ...
        {
            numAct[i] = 1;
            nextAct[i][0] = Pacman::stay;

            if (gameField.players[i].dead) continue;
            Pacman::Direction d2 = Pacman::shootUp;
            for (Pacman::Direction d = Pacman::up; d < Pacman::shootUp; ++d)
            {
                if (gameField.ActionValid(i, d))
                {
                    nextAct[i][numAct[i]++] = d;
                    if (gameField.ActionValid(i, d2))
                        nextAct[i][numAct[i]++] = d2;
                }
                ++d2;
            }
        }
    }
    double valueForState(int playerID, int act)
    {
        return val[playerID][act] / numSim[playerID][act];
    }
    int findChild(int actNext[4])
    {
        for (int i = 0; i < next.size(); i++)
        {
            state *temp = next[i];
            int j = 0;
            while (temp->lastMove[j] == actNext[j] && j < 4) j++;
            if (j == 4) return i;
        }
        return -1;  //如果没有返回-1
    }
...
state *selectForNextState(state *curState)
{
    while (true)
    {
        if (curState->endGame) return curState;
        double best[4] = {-1000, -1000, -1000, -1000};
        int bestnext[4] = {0, 0, 0, 0};
        for (int player = 0; player < 4; player++)
        {
            for (int act = 0; act < curState->numAct[player]; act++)
            {
                double temp;
                if (curState->numSim[player][act] > 0)
                    temp = curState->valueForState(player, act) +
                           sqrt(ALPHA * log(curState->numTotal) /
                                curState->numSim[player][act]);
                else
                    temp = 1000000;
                if (best[player] < temp)
                {
            ...
            gameField.actions[player] =
                curState->nextAct[player][bestnext[player]];
        }

        bool gameNext = gameField.NextTurn();
        count++;

        int indexNext = curState->findChild(bestnext);
        if (indexNext == -1)
        {
            state *nextState = new state;
            nextState->endGame = !gameNext;
            nextState->parent = curState;
            for (int i = 0; i < 4; i++) nextState->lastMove[i] = bestnext[i];
            curState->next.push_back(nextState);
            curState = nextState;
            return curState;
        }
        else
            curState = curState->next[indexNext];
    }
    return curState;
}

void rollout_RL(double res[4])
{
    // calculate value based on hand-crafted feature
    double res_1[4], res_2[4];
    for (int i = 0; i < 4; i++)
    {
        double feature[5], strength_self, strength_other = 0;
        double w1 = 10, w2 = -1;
        strength_self = gameField.players[i].strength;

        for (int j = 0; j < 4; j++)
        {
            if (i != j)
            {
                strength_other += gameField.players[j].strength;
            }
        }

        res_1[i] = w1 * strength_self + w2 * strength_other;
    }
    // calculate value based on random action
    int tcount = count;
    while (count - tcount < 5)
    {
        // 对每个玩家生成随机的合法动作
        for (int i = 0; i < MAX_PLAYER_COUNT; i++)
        {
            ...
            gameField.actions[i] = valid[rand() % vCount];
        }
    ...
    // 计算分数
    double total = 0.0001;
    for (int i = 0; i < 4; i++) res_2[i] = gameField.players[i].strength;
    for (int i = 0; i < 4; i++) res[i] = 0.5 * res_1[i] + 0.5 * res_2[i];
}

void rollout(double res[4])
{
    // calculate value based on random action
    int tcount = count;
    while (count - tcount < 5)
    ...
    // 计算分数
    double total = 0.0001;
    for (int i = 0; i < 4; i++) res[i] = gameField.players[i].strength;
}

void backProp(double result[4], state *curState)
{
    // Normalize
    double total = 0;
    for (int i = 0; i < 4; i++) total += result[i];
    if (total > 0)
        for (int i = 0; i < 4; i++) result[i] /= total;
    else
    {
        for (int i = 0; i < 4; i++) result[i] = 0.25;
    }
    // Back prop to update average value for each node
    while (true)
    {
        int last[4];
        for (int i = 0; i < 4; i++) last[i] = curState->lastMove[i];
        curState = curState->parent;
        if (curState == NULL) break;
        curState->numTotal++;
        for (int i = 0; i < 4; i++)
        {
            curState->numSim[i][last[i]]++;
            curState->val[i][last[i]] += result[i];
        }
    }
}

void decisionNE(state *root, int res[4])
{
    for (int i = 0; i < 4; i++)
    {
        double best = -1000;
        int bestChoice = -1;
        for (int k = 0; k < root->numAct[i]; k++)
        {
            double val = root->valueForState(i, k);
            if (val > best)
            {
                best = val;
                bestChoice = root->nextAct[i][k];
            }
        }
        res[i] = bestChoice;
    }
...
int totalExpanded;
int totalFree;
void freeNode(state *node)
{
    int t = node->next.size();
    while (t > 0)
    {
        state *temp = node->next[t - 1];
        node->next.pop_back();
        freeNode(temp);
        t = node->next.size();
    }
...
void MCTS(int res[4])
{
    ...
    dbegin = clock();
    while (clock() - dbegin < 100000)
    {
        state *cur = root;
        count = 0;
        cur = selectForNextState(cur);
        double res[4];
        if (cur->endGame)
            for (int i = 0; i < 4; i++) res[i] = gameField.players[i].strength;
        else
    ...
    totalExpanded = root->numTotal;
    decisionNE(root, res);
}

int MCTS_RL(int myID)
{
    root = new state;
    root->parent = NULL;
    int temp_id[4];
...
{
    int myID = 0;
    int evaluate_round = 20;  // total number of rounds to play
    int max_step = 100;       // maximum number of step within one round
    bool print_info = 0;      // if print debug information
    ...
    gameField.SKILL_COST = 6;
    gameField.GENERATOR_INTERVAL = 20;

    // Pacman::PrepareInitialField(staticField, contentField);
    int survive[MAX_PLAYER_COUNT] = {0};
    float average_score[MAX_PLAYER_COUNT] = {0};
    std::ofstream fout;
    fout.open(argv[1], std::ofstream::out | std::ofstream::trunc);
    if (!fout.good()) throw "I/O error";
    for (int num_game = 0; num_game < evaluate_round; num_game++)
    {
        gameField.turnID = 0;
        InitializeField(gameField);
        gameField.setDistFruit = false;
        gameField.bfsFruit();
        for (int round = 0; round < max_step; round++)
        {
            int maxD[4];
            MCTS(maxD);
            totalFree = 0;
            ...
            for (int _ = 0; _ < MAX_PLAYER_COUNT; _++)
            {
                gameField.actions[_] = (Pacman::Direction)(maxD[_]);
                if (!gameField.players[_].dead)
                {
                    if (print_info)
                        cout << "Player " << _ << " action" << maxD[_] << endl;
                    fout << gameField.height << " " << gameField.width << " "
                         << gameField.GENERATOR_INTERVAL << " "
                         << gameField.SKILL_COST << " "
                         << gameField.LARGE_FRUIT_DURATION << " "
                         << gameField.LARGE_FRUIT_ENHANCEMENT << " "
                         << gameField.smallFruitCount << " ";
                    fout << gameField.turnID << " "
                         << gameField.generatorTurnLeft << " "
                         << gameField.players[_].dead << " "
                         << gameField.players[_].strength << " "
                         << gameField.players[_].powerUpLeft << " "
                         << gameField.players[_].col << " "
                         << gameField.players[_].row << " ";
                    for (Pacman::Direction k = Pacman::stay; k < 8; ++k)
                    {
                        if (gameField.ActionValid(_, k))
                            fout << 1 << " ";
                        else
                            fout << 0 << " ";
                    }
                    ...
                    {
                        if (k != _)
                            fout << gameField.players[_].dead << " "
                                 << gameField.players[k].strength << " "
                                 << gameField.players[_].powerUpLeft << " "
                                 << gameField.players[k].col << " "
                                 << gameField.players[k].row << " ";
                    }
                    // char *Buffer1 = (char*) malloc(200*sizeof(char));
                    // char *Buffer2 = (char*) malloc(200*sizeof(char));
                    for (int r = 0; r < FIELD_MAX_HEIGHT; r++)
                    {
                        for (int c = 0; c < FIELD_MAX_WIDTH; c++)
                        {
                            // sprintf(Buffer1 + strlen(Buffer1),"%d ",
                            // gameField.fieldContent[r][c]);
                            // sprintf(Buffer2 + strlen(Buffer2),"%d ",
                            // gameField.fieldStatic[r][c]);
                            fout << gameField.fieldContent[r][c] << " ";
                        }
                    }
                    for (int r = 0; r < FIELD_MAX_HEIGHT; r++)
                    {
                        for (int c = 0; c < FIELD_MAX_WIDTH; c++)
                        {
                            ...
                            // sprintf(Buffer2 + strlen(Buffer2),"%d ",
                            // gameField.fieldStatic[r][c]);
                            fout << gameField.fieldStatic[r][c] << " ";
                        }
                    }
                    // fout << Buffer1 << Buffer2;
                    fout << maxD[_] << " " << endl;
                }
            }

            if (gameField.NextTurn() && round < max_step - 1)
                cout << round << endl;
            else
            {
                if (print_info) gameField.DebugPrint();
                for (int k = 0; k < MAX_PLAYER_COUNT; k++)
                {
                    if (!gameField.players[k].dead) survive[k]++;
                    average_score[k] += gameField.players[k].strength;
                }
                if (print_info) cout << "Round " << num_game << " over" << endl;
                break;
            }
            if (print_info) gameField.DebugPrint();
        }
    ...
    {
        cout << "Player " << k
             << " survive percentage: " << survive[k] / float(evaluate_round)
             << " average strengh remains: "
             << average_score[k] / float(evaluate_round) << endl;
    }
    fout.close();
    return 0;
}
